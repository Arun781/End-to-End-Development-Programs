package com.ygt.sources.controller;
...
import com.ygt.sources.services.YGTAppService;


@Controller
@RequestMapping("/")
public class YgtAppController {
	@Autowired
	private YGTAppService appService;
	
	private int otp;

	public YgtAppController() {
	...
	@PostMapping("/bankApp")
	public String onBank(Model model, YgtDto appDTO, @RequestParam String password, @RequestParam String reenterpas,
...
//			return "CustomerReg";
//		} else {
		Set<ConstraintViolation<YgtDto>> violations = this.appService.validateAndSave(appDTO);
	...
	public String onSearch(@RequestParam int id, Model model) {
		...
		YgtDto appDTO = this.appService.findBy(id);
		if (appDTO != null) {
	...
	public String onFindByName(Model model, @RequestParam String name) {
		...
		List<YgtDto> list = this.appService.findByName(name);
	...
	public String onFindByBranch(Model model, @RequestParam String branch) {
		...
		List<YgtDto> list = this.appService.findByBranch(branch);
		System.out.println("List Size in controller" + list.size());
		if (list != null && !list.isEmpty()) {
	...
	@PostMapping("/update")
	public String onUpdate(Model model, YgtDto appDTO) {
		System.out.println("Running onUpdate post");
		Set<ConstraintViolation<YgtDto>> violtion = this.appService.update(appDTO);
		if (!violtion.isEmpty()) {
	...
	public String onLogin(@RequestParam String email, @RequestParam String password, Model model) {
		...
		YgtDto dto = this.appService.findByEmail(email);
	...
	public String onFindAll(Model model) {
		...
		List<YgtDto> dto = this.appService.findAll();
	...
	public String findByNameAndBranch(@RequestParam String name, @RequestParam String branch, Model model) {
		...
		if (!name.isEmpty() && branch.isEmpty()) {
			List<YgtDto> dtoname = this.appService.findByName(name);
			if (dtoname.size() != 0) {
			...
			List<YgtDto> dtoLocation = this.appService.findByBranch(branch);
			...
			List<YgtDto> dto = this.appService.findByNameAndBranch(name, branch);
			if (dto != null) {
	...
	@PostMapping("/reset")
	public String onPass(Model model, YgtDto appDTO, @RequestParam String password, 
			@RequestParam String reenterpas) {
		System.out.println("Running the onPass in Controller...");
		List<YgtDto> violation = this.appService.validateAndSavePass(password, reenterpas);
		...
		violation.forEach(cv -> System.out.println(((ConstraintViolation<YgtDto>) cv).getMessage()));
